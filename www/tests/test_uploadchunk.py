...
def test_login_required(client, auth):
    auth.logout()
    response = client.get('/uploadchunk/')
    assert response.status_code == 302
    assert response.headers['Location'] == 'http://localhost/auth/login'
    response = client.post('/uploadchunk/')
    assert response.status_code == 302
    assert response.headers['Location'] == 'http://localhost/auth/login'
...
def test_redirect(client, auth):
    auth.logout()
    response = client.get('/uploadchunk', follow_redirects=False)
    assert response.status_code == 301
    assert response.headers['Location'] == 'http://localhost/uploadchunk/'
    response = client.post('/uploadchunk', follow_redirects=False)
    assert response.status_code == 301
    assert response.headers['Location'] == 'http://localhost/uploadchunk/'
...
def test_post(client, auth):
    auth.login()
    assert client.post('/uploadchunk/').status_code == 200
