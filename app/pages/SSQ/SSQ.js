...
class SSQ extends React.Component {
  ...
  formIsFilled() {
    ...
    allQuestionsAnswered = Object.values(this.state.symptoms)
        ...
        <ScrollView>
          {SSQVars.symptoms.map((symptom) =>
            <Card>
              ...
              <ButtonGroup
                onPress={(index) => this.setState({ symptoms: {...this.state.symptoms, [symptom.index]: index} }) }
              selectedIndex={this.state.symptoms[symptom.index]}
              selectedButtonStyle={{ backgroundColor: `${buttonColor[this.state.symptoms[symptom.index]]}` }}
...
export default connect(mapStateToProps, null)(SSQ);
